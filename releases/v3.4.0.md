...
* Fixed an issue when using the restart driver flag on Android would cause the execution to fail.
* Fixed an issue with test imports.
* Fixed an issue when the execution progress bar on the monitor tab reach above 100%.

#### Addons:

Added Robot Operations check out the [documentation here](../testproject-addons/available-addons/robot-operations-addon.md).
